...
- $\mathsf{X}$：张量
- $\mathrm{I}$：单位矩阵
- $x_i,[\mathrm{x}]_i$：向量 $\mathrm{x}$ 的第 $i$ 个元素
- $x_{ij},[\mathrm{X}]_{ij}$：矩阵 $\mathrm{X}$ 第 $i$ 行第 $j$ 列的元素
...
- $\sum$: 连加
- $\prod$: 连乘
- $\overset{\mathrm{def}}{=}$: 定义

## 微积分
- $\dfrac{dy}{dx}$: $y$关于$x$的导数
- $\dfrac{\partial y}{\partial x}$: $y$关于$x$的偏导数
...
- $p(x)$: 概率密度函数
- $E_{x}\big[f(x)\big]$: 函数$f$对$x$的数学期望
...
- $H(X)$: 随机变量$X$的熵
- $D_{\mathrm{KL}}(P \parallel Q)$: $P$和$Q$的KL-散度
