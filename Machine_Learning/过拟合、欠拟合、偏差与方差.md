...
- 一种是学的太死板，只会生搬硬套例题（**欠拟合**）。
- 另一种是学的太聪明，把例题的标点符号甚至笔迹特点都背下来了，导致面对新题时不知所措（**过拟合**）。

理解**过拟合**和**欠拟合**，以及其背后更深层的理论概念——**偏差**与**方差**，是每一位机器学习实践者从入门走向精通的关键一步。它们解释了模型为何会犯错，并为我们指明了模型改进的方向。
...
我们的目标是找到一条曲线（模型），能最好地描述这些蓝色散点（数据）所反映地规律。模型对数据的描述程度，就是**拟合**。

![](../assets/2026-06-15-01-11-53.png)

## 1、欠拟合
**欠拟合**是指模型过于简单，无法捕捉数据中的基础规律或模式。就像一个学生只学了加法，却要去解微积分题目。
...
- **表现**：在训练集和测试集上的误差都比较低，且两者接近。
- **类比**：用一个适当结束的多项式（例如3阶）来拟合数据。
...
## 3.过拟合
**过拟合**是指模型过于复杂，不仅学习了数据中的真是规律，还“记住”了训练数据中的随机噪声和异常值。
...
![](../assets/2026-06-15-01-28-56.png)
...
![](../assets/2026-06-15-16-44-09.png)
...
![](../assets/2026-06-15-16-50-55.png)
![](../assets/2026-06-15-16-51-02.png)
![](../assets/2026-06-15-16-51-10.png)
![](../assets/2026-06-15-16-51-17.png)
...
- **增加正则化**：
  ...
  - **L2正则化（Ridge）**：使权重衰减，倾向于让所有权重都减小。
  - **Dropout（用于神经网络）**：在训练中随机“丢弃”一部分神经元。
- **早停**（用于迭代模型）：当验证集误差不在下降时停止训练。
...
![](../assets/2026-06-15-17-09-02.png)
